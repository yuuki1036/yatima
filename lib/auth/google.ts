...
// YAT-34: Google OAuth（Authorization Code フロー）。YAT-39 でパスワード認証を撤去し、これが
// 唯一のログイン手段になった。「Google でログイン → ID トークンをサーバ検証 → 所有者メールなら
// createSession() で JWT Cookie を発行」する軽量方式。proxy / requireSession / logout / セッション
// 発行は共通なので再利用する。単一所有者アプリなので「許可メール = OWNER_EMAIL 1 件」で十分
// （ユーザーテーブルは持たない）。新依存は足さず jose（既存）で ID トークンを検証する。
...
// env が3点そろっていれば設定を返す（1つでも欠ければ null＝Google ログイン不能。唯一の手段なので
// 呼び出し側は /login に設定エラーを出す）。
