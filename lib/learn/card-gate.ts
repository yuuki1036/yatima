...
// を card 経路へ転記するもの。英語記事の逐語引用×日本語カードで固有トークンが言語違いによりほぼ
// 重ならず、jaccard が構造的に 0 へ潰れるため閾値較正では解決しない。
// 計測（YAT-58 / diagnose-card-grounding・サンプル 6 記事・生成 25 件）: low_overlap が棄却の
// 68%、通過率 12.0% → ④無効で 80.0%。jaccard 実測値は 0.000〜0.054 で qa/cloze とも一様に棄却。
// ②逐語＋③固有性が「引用は実在の記事固有テキスト」を担保する。この経路では②が現に 25 件中 5 件
// （20%）を捕捉しており、MCQ（同計測で②棄却 0 件）より防御が効いている。
// 副作用は MCQ と同じ: quote が設問と無関係な箇所から選ばれる余地が残る。カードは承認制なので
// 人のレビューが緩衝する（LLM 生成カードを決定的フィルタで囲う設計の前提）。
...
function isGrounded(card: GeneratedCard, groundBody: string): boolean {
  ...
  return isQuoteGrounded(card.source_quote, groundBody, target, CARD_MIN_OVERLAP);
}
