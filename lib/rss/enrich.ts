...
const THIN_BODY_CHARS = 300; // htmlToInputText 後がこれ未満なら「薄い」= 本文取得を試みる
...
export async function enrichMissingBodies(
  ...
  const targets = rows.filter(
    (r) => r.url && htmlToInputText(r.content_html).length < THIN_BODY_CHARS,
  );
  ...
  for (let i = 0; i < targets.length; i += concurrency) {
    ...
    const results = await Promise.allSettled(
      chunk.map(async (row) => {
        ...
        if (
          htmlToInputText(content).length <=
          htmlToInputText(row.content_html).length
        ) {
          throw new Error("取得本文が元と同等以下");
        }
