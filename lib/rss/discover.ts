...
// SSRF ガード。候補 URL は第三者の記事リンク由来で信頼できないため、http/https 以外と
// 内部・予約レンジ（localhost / ループバック / リンクローカル 169.254 / RFC1918 / CGN /
// ユニークローカル IPv6）を弾く。通れば正規化済み URL を、弾けば null を返す。
// 名前解決後の IP 判定（DNS リバインディング厳密対策）まではやらず、最安の一次防御に絞る。
function isPubliclyRoutableHttpUrl(raw: string): URL | null {
  let u: URL;
  try {
    u = new URL(raw);
  } catch {
