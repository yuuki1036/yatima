...
const SYSTEM_PROMPT = [
  "あなたは技術記事の要約者です。",
  "与えられた記事の要点を日本語で要約してください。字数は固定せず、記事の情報量に応じた自然な長さで書くこと（内容が薄ければ簡潔に、情報量が多ければ厚めに）。",
  "カードを開かなくても概要が掴めるよう、何が・なぜ重要か・具体（固有名詞や数値など）を漏らさず述べること。「この記事は」などの前置きや感想は書かないこと。",
  "本文が乏しい（タイトルやリンクのみ等）場合は、推測で埋めず分かる範囲だけを簡潔に書くこと。",
...
const ANNOTATE_SYSTEM_PROMPT = [
  ...
  '{"summary": "...", "tags": ["...", "..."]}',
  "summary: 字数は固定せず、記事の情報量に応じた自然な長さで。カードを開かなくても概要が掴めるよう、何が・なぜ重要か・具体（固有名詞や数値など）を漏らさず述べる。本文が乏しい場合は推測で埋めず分かる範囲だけ簡潔に。「この記事は」等の前置きや感想は書かず、体言止めも可。マークダウンやラベルは付けない。",
...
class HaikuSummarizer implements Summarizer {
  ...
  async summarize(input: SummarizeInput): Promise<string> {
    ...
    const res = await this.client.messages.create({
      model: MODEL,
      max_tokens: 1200, // 字数固定なし。情報量の多い記事の長めの要約に余裕を持たせる
  ...
  async annotate(input: SummarizeInput): Promise<Annotation> {
    ...
    const res = await this.client.messages.create({
      model: MODEL,
      max_tokens: 1400, // 字数固定なしの長め要約 + tags の JSON 分のマージン（途中切れ防止）
      thinking: { type: "disabled" },
