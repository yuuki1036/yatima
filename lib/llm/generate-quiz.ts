...
function buildSystemPrompt(input: GenerateQuizInput): string {
  ...
  return [
    "あなたは技術の学習ソース（公式ドキュメント・定番の解説）からエンジニア向けの選択式クイズ（MCQ）を作る出題者です。",
    `出題テーマの目安: ${input.categoryLabel}。`,
    "与えられた本文から、応用理解を問う4択問題を作ってください。",
    "定義の丸暗記ではなく「いつ・なぜ・どう使うか / 何が壊れるか / どんなトレードオフか」を問うこと。",
    "時事的な事実（いつ・誰が・どのバージョンを発表したか等の日付・固有の出来事）は設問化せず、",
    "いつ学んでも価値のある概念・仕組み・原則を問うこと。",
    `問題は1ソースにつき最大 ${MAX_MCQ_PER_ARTICLE} 問。1 問 1 概念で互いに重複させないこと。`,
    "各問題は choices を必ず4件にし、そのうち1件だけが正解。answer_index は正解の 0-based 位置。",
    "誤答（distractor）は「もっともらしいが明確に誤り」にすること。自明・重複・的外れな選択肢は避ける。",
    "各問題には必ず source_quote を付けること。source_quote は本文から文字単位でそのまま",
    "コピー＆ペーストした 1 文程度の抜粋にする。本文が英語なら英語のまま抜き出し、翻訳・言い換え・",
    "語順変更・要約・誤字修正・記号や引用符の置換を一切しないこと（設問や解説は日本語でよいが、",
    "source_quote だけは原文の文字列と完全一致させる）。該当箇所を原文からそのまま貼り付けること。",
