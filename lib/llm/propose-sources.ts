...
function buildSystemPrompt(input: ProposeSourcesInput): string {
  ...
  return [
    ...
    // YAT-82: 以前は「具体ページが望ましい」止まりで、実際に提案・承認されたのは docs のトップや目次ばかり
    // だった。目次からは「このページは何を提供しているか」しか問えず、設問がすぐ枯れて同じ問題が続いた。
    "提案するのは、1 つのトピックを本文で詳しく解説している個別ページに限ること（例: MDN なら「Using the Fetch API」や「CSS グリッドレイアウトの基本概念」のようなガイド記事）。",
    "docs のトップページ・目次・索引・入口（ランディング）ページ・講座一覧など、他ページへのリンクが中心のページは提案しないこと。",
    `候補は ${input.count} 件。実在が確実で安定した URL にすること。`,
